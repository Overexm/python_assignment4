...
def refresh():
    ...
    for i in paragraphs:
        ...
        some_name = ""
        some_market = 0
        for j in i.find_all("td")[2:7]:
            
            if j.get('a')is not None:
                some_name = j.get('a').text
            elif j.find('span',class_="sc-1ow4cwt-0") is not None:
                some_v = j.find('span',class_="sc-1ow4cwt-0").text.replace('$',"")
                some_v = some_v.replace('T',"")
                #some_v = some_v.replace('.',",")
                some_v = some_v.replace('B',"")
                para_1.marketCap = float(some_v)
            else:
                #print(j.text)
                print()
